...
include('includes/admin_top.php'); 
    ...
    $rc=mysql_fetch_assoc(mysql_query("SELECT * FROM ".TABLE_CHALLAN." WHERE id='$editid'"));
     $rc_no=$rc['no'];
    if(isset($_POST['update_banner']) && $_POST['update_banner']=='update_banner'){
        ...
        $_POST['challan_no']=$q;
            $db->updateArray(TABLE_CHALLAN,$_POST, "id=".$editid) or die(mysql_error());
            ...
            for($count=0;$count<count($checkbox1);$count++)
        {
            ...
            //$chk.=$count;
            if($editid2!=''){
            $db->updateArray(TABLE_CHALLAN_PART,$data, "id=".$editid2) or die(mysql_error());
        }else{
            $data['challan_id']=$editid;
            $db->insertDataArray(TABLE_CHALLAN_PART,$data);
        }
        }
    ...
    $get_des = $pm->getTableDetails(TABLE_CHALLAN,'id',$editid);
...
<br>
        ...
        <tbody class="itm_po_tbl">
            ...
            $i=0;
            $j='';
            $sql_part = "SELECT * FROM ".TABLE_CHALLAN_PART." where challan_id='$editid' ORDER BY id ASC";
            ...
            <div  style="float: right;margin-right: 20px; cursor: pointer;" onclick="add_item_po();">
                ...
                <div class="box-footer">                    
                    <a href="item_challan_list.php" type="button" class="btn btn-info">Close</a>
